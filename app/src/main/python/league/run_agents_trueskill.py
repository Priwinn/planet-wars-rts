    ...
    with Session(ENGINE) as session:
        ...
        for i in range(n_pairs):
            ...
            try:
                ...
                id1, id2 = choose_next_pair(session, league_id)
                a = session.query(Agent).filter_by(agent_id=id1).first()
                b = session.query(Agent).filter_by(agent_id=id2).first()
                inst_a = session.query(AgentInstance).filter_by(agent_id=a.agent_id).first()
                inst_b = session.query(AgentInstance).filter_by(agent_id=b.agent_id).first()
                if not all([a, b, inst_a, inst_b]):
                    raise RuntimeError("Failed to load agents or instances from DB for selected pair")
