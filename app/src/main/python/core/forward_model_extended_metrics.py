...
class ForwardModelWithMetrics:
    ...
    def __init__(self, state: GameState, params: GameParams):
        self.state = state
        self.params = params
        self.player_conquers = {Player.Player1: {'opponent': 0, 'neutral': 0}, Player.Player2: {'opponent': 0, 'neutral': 0}}
    ...
    def step(self, actions: Dict[Player, Action]):
        ...
        ForwardModelWithMetrics.n_updates += 1
    ...
    def apply_actions(self, actions: Dict[Player, Action]):
        for player, action in actions.items():
            ...
            if source.transporter is None and source.owner == player and source.n_ships >= action.num_ships:
                ...
                ForwardModelWithMetrics.n_actions += 1
            else:
                ForwardModelWithMetrics.n_failed_actions += 1
    ...
    def update_neutral_planet(self, planet: Planet, pending: Optional[Dict[Player, float]]):
        ...
        if planet.n_ships < 0:
            ...
            if net > 0:
                self.player_conquers[Player.Player1]['neutral'] += 1
            else:
                self.player_conquers[Player.Player2]['neutral'] += 1
    ...
    def update_player_planet(self, planet: Planet, pending: Optional[Dict[Player, float]]):
        ...
        if planet.n_ships < 0:
            ...
            planet.n_ships = -planet.n_ships
            self.player_conquers[planet.owner]['opponent'] += 1
    ...
    def reset_metrics(self):
        ForwardModelWithMetrics.n_updates = 0
        ForwardModelWithMetrics.n_failed_actions = 0
        ForwardModelWithMetrics.n_actions = 0
        self.player_conquers = {Player.Player1: {'opponent': 0, 'neutral': 0}, Player.Player2: {'opponent': 0, 'neutral': 0}}
